...
<div align="center">
  <img src="https://res.cloudinary.com/ds64xs2lp/image/upload/v1787480189/v1-0-3-release_lsbajc.svg" alt="Numerology API v1.0.3 Changelog" style="max-width: 100%; border-radius: 12px; margin: 1em 0;">
</div>

**Release date:** 2026-08-22
**Version:** v1.0.3
**Scope:** Zodiac sign intelligence, planetary report reliability, Birth Day corrections, on-demand translation providers, and the first Numora Studio desktop beta.
...
- Added dedicated Zodiac Sun Sign, Moon Sign, Rising Sign, Modality, and Element endpoints.
...
- Fixed Birth Day Number request and meaning-resolution issues while preserving GET and POST parity.
...
- Added full on-demand translation routing for horoscope-report responses, with self-hosted LibreTranslate, Google Translate, and DeepL BYOK support.
- Added provider observability through `_api_metadata_.translation_provider` for translated responses.
- Released Numora Studio v0.1.0-beta for macOS, Windows, and Linux practitioners.
- Bumped the runtime FastAPI/OpenAPI version and standardized response metadata on v1.0.3.

## New

### Zodiac sign intelligence

The API now exposes five dedicated zodiac reference families under `/api/v1/horoscope/sign/`. Each family is authenticated, usage-tracked, localized through the shared response pipeline, and available through both `GET` and `POST` where the endpoint contract supports a body request.
...
All five families return structured sign facts, symbols, archetypes, summaries, detailed meanings, and an enterprise usage wrapper. They are designed to work independently for lightweight integrations and alongside birth-chart or horoscope-report workflows for richer practitioner experiences.

### Translation provider routing

Horoscope report responses can now be translated on demand without requiring a separate pre-generated payload for every request. The translator can be selected per request with the `translator` query parameter or the `X-Translator-Provider` header:

- **LibreTranslate** — the default self-hosted provider for production report translation.
- **Google Translate** — the built-in Google-backed provider for supported language pairs.
- **DeepL BYOK** — use your own DeepL key with `translator=deepl` and the `X-Translator-Api-Key` header.

The translator used for a non-English response is reported in `_api_metadata_.translation_provider`. Existing language negotiation remains compatible with the `lang` query parameter and the `Accept-Language` header.

### Birth-chart SVG wheel customization

The `/api/v1/birth-chart/svg` endpoint now accepts a complete `wheel` customization object instead of limiting integrations to a small set of visual aliases. Clients can select a supported preset or theme, override the palette and accent color, control panel layout and opacity, toggle chart labels and visual layers, set title and subtitle text, provide per-planet colors, and choose split, transparent, or compressed SVG output. Existing top-level theme aliases remain accepted for compatibility.

### Numora Studio v0.1.0-beta

The first practitioner desktop release is now available as **Numora Studio v0.1.0-beta** for macOS, Windows, and Linux. Numora Studio uses the API's dedicated Studio channel and supports the new zodiac and horoscope-report capabilities in a practitioner-oriented workspace. Studio requests identify themselves with `X-Numora-Client: numora-studio/<version>` and use `tnsr_` keys so Studio usage is metered independently from platform `tnea_` keys.

## Fixes and compatibility

- Added `general` to the canonical planetary option set and made it the default in planetary section and overview request models.
- Corrected date parsing and validation for the Birth Day `YYYY-MM-DD` request contract and fixed detailed meaning lookup.
- Preserved GET and POST behavior, authentication, usage tracking, enterprise metadata, and localized response formatting for Birth Day requests.
- Standardized the runtime version exposed by FastAPI/OpenAPI and shared response metadata to `1.0.3`.
- Preserved existing birth-chart SVG clients; new integrations can progressively adopt the `wheel` object for full visual control.
- No production API endpoint was deprecated in v1.0.3.

## Documentation and validation

- Added the v1.0.3 release note to the dashboard's internal MkDocs reference.
- Refreshed the public documentation release index and localized release pages.
- Added Numora Studio imagery to the release documentation.
- Documented the full birth-chart SVG wheel customization contract and compatibility aliases.
- Validated zodiac route registration, planetary request defaults, response metadata, translation-provider reporting, and Birth Day GET/POST contracts.

### Related links

- [Numora Studio desktop repository](https://github.com/dakidarts/numora-studio-desktop)
- [Numora Studio product page](https://numerologyapi.com/studio)
- [v1.0.3 documentation](https://docs.numerologyapi.com/changelog/changelog-v1-0-3/)
